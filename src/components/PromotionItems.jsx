...
//     useEffect(()=>{
//         axios.get("http://localhost:5000/api/promotionproducts")
//         .then(res=>{
//             setItems(res.data);
//         }).catch(err=>{
//             console.log(err);
...
//                         <h3 className="display-6 text-center">Promotion Products</h3>
                        ...
                        <div className="row g-2">
                            {
                                items.map(item=>{
                                    const discountPct = item.originalprice
                                        ? Math.round(100 - (Number(item.price) / Number(item.originalprice)) * 100)
                                        : 0;
                                    ...
                                    <div className="col-lg-2 col-md-4 col-sm-6" key={item.id}>
                                        <div className="card h-100 shadow-md" style={{cursor:"pointer"}} onClick={()=>navigate(`/allproducts/${item.id}`)}>
                                            <div className="position-relative">
                                                {discountPct > 0 && (
                                                    <span className="badge bg-danger position-absolute" style={{top:"8px",left:"8px"}}>
                                                        -{discountPct}%
                                                    </span>
                                                )}
                                                <img src={item.image} className="card-img-top p-2" alt={item.title} style={{maxHeight:"150px",objectFit:"contain"}} />
